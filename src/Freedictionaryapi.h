#ifndef FREEDICTIONARYAPI_H
#define FREEDICTIONARYAPI_H
...
private:
    std::string api_base = "https://freedictionaryapi.com/api/v1";
    ...
    http_request create_request(const std::string& endpoint, const std::string& method, const std::string& data = "") {
        ...
        // Set headers
        request.headers().add(U("Host"), U("freedictionaryapi.com"));
...
public:
    ...
    pplx::task<json::value> get_entries(const std::string& lang, const std::string& word,bool translations=false) {
        std::map<std::string, std::string> params;
        if (translations) params["translations"] = translations;
        return make_api_call("/entries/" + utility::conversions::to_utf8string(web::uri::encode_data_string(utility::conversions::to_string_t(lang))) + "/" + utility::conversions::to_utf8string(web::uri::encode_data_string(utility::conversions::to_string_t(word))) + build_query_params(params),"GET");
    }
};
